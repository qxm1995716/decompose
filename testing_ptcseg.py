...
import torch.optim
from dataloader.Infer_ScanDict import scan_dict
...
def args():
    ...
    paras.add_argument('--ptc_block_nums', type=list, default=[3, 4, 3], help='The number of transformer blocks in ptc branch.')
    ...
    paras.add_argument('--s2d_v_res', type=float, default=0.5, help='The resolution in vertical direction (height direction).')
...
def testing(args, log):
    ...
    # loading weight params
    weight_params = torch.load(r'./ckpt.pt')
    ...
    with torch.no_grad():
        ...
        for t_iters, (norm_xy, utm_xyz, feat, mask, gt, s2dmap, dm_coord, f_name, raw) in enumerate(testing_dataset):
            ...
            acc_stat += _acc
            print_str = 'TESTING - EPOCH-> ITER->[{:d} / {:d}]: MIOU is {:.6f}; '.format(
                t_iters + 1, len(testing_dataset), mean_iou)
        ...
        log.log_string(
            '######################## TESTING STATISTIC ANALYZE#########################')
        print_str = '---> [TESTING] MIOU is {:.6f}; '.format(F_MIOU)
    ...
    # 保存训练文件
    shutil.copy('testing_rtsv2.py', str(log.log_dir))


if __name__ == '__main__':
    args = args()
    log = logger('DECOMPOSE_TESTING')
    testing(args, log)
